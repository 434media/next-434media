...
import { Loader2, Mail, Pause, Play, Square, Send, ListChecks, CheckCircle2, ChevronDown } from "lucide-react"
...
export function OutreachSequencePanel({ lead, isEditing, onSequenceAction }: Props) {
  ...
  // Which draft cards are expanded for editing. Collapsed by default so the
  // review reads as a scannable 3-email summary, not a wall of textareas.
  ...
  const setup = async () => {
    ...
    if (r.ok && r.steps) {
      setDrafts(r.steps)
      setExpandedSteps(new Set()) // start collapsed — review as a summary first
    }
  }
  const enroll = async () => {
    if (!drafts) return
    ...
    return (
      <div className="mt-3 space-y-2.5">
        <p className="text-[11px] text-neutral-500">
          Review the 3 emails — they send on a 0 / +4 / +5 business-day cadence after you enroll. Tap any email to edit
          it before sending. Stops automatically on reply (mark the lead engaged), opt-out, or convert.
        </p>
        {drafts.map((d, i) => {
          const isOpen = expandedSteps.has(i)
          return (
            <div key={d.n} className="rounded-md border border-neutral-200 overflow-hidden">
              {/* Collapsed header — scannable summary; click to edit */}
              <button
                type="button"
                onClick={() => toggleStep(i)}
                aria-expanded={isOpen}
                className="w-full flex items-start gap-2 px-2.5 py-2 text-left hover:bg-neutral-50"
              >
                <span className="mt-0.5 inline-flex items-center justify-center w-4 h-4 shrink-0 rounded-full bg-neutral-900 text-white text-[9px] font-bold tabular-nums">
                  {d.n}
                </span>
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-[12px] font-medium text-neutral-800 truncate">
                    {d.subject || <span className="italic text-neutral-400">No subject</span>}
                  </span>
                  {!isOpen && (
                    <span className="block text-[11px] text-neutral-500 truncate">{d.body}</span>
                  )}
                </span>
                <ChevronDown
                  className={`mt-0.5 w-3.5 h-3.5 shrink-0 text-neutral-400 transition-transform ${isOpen ? "rotate-180" : ""}`}
                  aria-hidden
                />
              </button>
              {/* Expanded — editable subject + body */}
              {isOpen && (
                <div className="px-2.5 pb-2.5 border-t border-neutral-100 pt-2">
                  <input
                    type="text"
                    value={d.subject}
