/**
 * Migration Script: Migrate Feed Data from media-analytics-proxy to groovy-ego
 * 
 * This script reads feed data from the source Firebase project (media-analytics-proxy)
 * and migrates missing records to the destination project (groovy-ego-462522-v2).
 * 
 * Usage:
 *   npx ts-node --esm scripts/migrate-feed-data.ts
 * 
 * Or with tsx:
 *   npx tsx scripts/migrate-feed-data.ts
 */
...
const COLLECTIONS_TO_MIGRATE = [
  "feed",           // THEFEED table
  "feed_8count",    // 8COUNT table  
  "feed_culturedeck", // CULTUREDECK table
]
  ...
  try {
    ...
    for (const doc of sourceSnapshot.docs) {
      ...
      try {
        ...
        console.log(`   ✓ Queued: ${docId} - ${data.title || data.slug || "Untitled"}`)
...
async function main() {
  ...
  console.log("  Feed Data Migration: media-analytics-proxy → groovy-ego")
  console.log("═".repeat(60))
  console.log()
