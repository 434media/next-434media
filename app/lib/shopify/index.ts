...
const reshapeCart = (shopifyCart: any): Cart => {
  if (!shopifyCart?.cost?.totalTaxAmount) {
    shopifyCart.cost.totalTaxAmount = {
      amount: '0.0',
      currencyCode: shopifyCart.cost.totalAmount.currencyCode
    };
  }

  // Ensure all required cost properties are present
  if (!shopifyCart.cost.subtotalAmount) {
...
const reshapeProducts = (products: any[]): Product[] => {
...
// Update the createCart function to ensure it properly sets up the checkout URL
...
export async function createCart(): Promise<Cart> {
  try {
    ...
    // Check if the cart was created successfully and has a checkout URL
    if (!res.body.data.cartCreate.cart || !res.body.data.cartCreate.cart.checkoutUrl) {
...
// Update the getCart function to handle errors better and ensure we have a valid cart
...
export async function getCart(): Promise<Cart | undefined> {
  try {
    ...
    if (!res.body.data.cart) {
      // Remove the invalid cart ID cookie
      (await cookies()).delete('cartId');
      return undefined;
    }

    // Ensure the cart has a checkout URL
    if (!res.body.data.cart.checkoutUrl) {
      console.warn("Cart missing checkout URL, creating a new cart");
      // Remove the invalid cart ID cookie
      (await cookies()).delete('cartId');
      return undefined;
    }
    ...
    // On error, clear the cart cookie to force creation of a new cart
    (await cookies()).delete('cartId');
    return undefined;
  }
}
