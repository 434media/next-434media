...
export async function GET(request: NextRequest) {
      ...
      return NextResponse.json(
        {
          ...
          help: {
            ...
            required: [
              ...
              "FACEBOOK_PAGE_ID_MILCITY - Your Facebook Page ID connected to Instagram",
  ...
  // Use MilCityUSA token from Vemos Vamos Insights App
  const accessToken = process.env.INSTAGRAM_ACCESS_TOKEN_MILCITY!
  const pageId = process.env.FACEBOOK_PAGE_ID_MILCITY!
  const businessAccountId = process.env.INSTAGRAM_BUSINESS_ACCOUNT_ID_MILCITY
    ...
    const resolveInstagramBusinessAccountId = async (): Promise<string> => {
      ...
      if (businessAccountId) {
        ...
        console.warn(
          `[Instagram API] Provided INSTAGRAM_BUSINESS_ACCOUNT_ID_MILCITY is ${note}. Falling back to resolving via FACEBOOK_PAGE_ID_MILCITY.`,
        )
      }
          ...
          const responseBody: any = {
            ...
            suggestions: missingPermissions.length
              ? [
                  ...
                  "Alternatively, set a valid INSTAGRAM_BUSINESS_ACCOUNT_ID_MILCITY to avoid the Page lookup.",
                ]
              : undefined,
            timestamp: new Date().toISOString(),
          }
