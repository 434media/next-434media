...
export async function GET(request: NextRequest) {
      ...
      return NextResponse.json(
        {
          ...
          help: {
            ...
            required: [
              "INSTAGRAM_ACCESS_TOKEN_VEMOS - Your Instagram Business Account access token",
              "FACEBOOK_PAGE_ID_VEMOS - Your Facebook Page ID connected to Instagram",
  ...
  // Use Vemos Vamos Insights App token
  const accessToken = process.env.INSTAGRAM_ACCESS_TOKEN_VEMOS!
  const pageId = process.env.FACEBOOK_PAGE_ID_VEMOS!
  const businessAccountId = process.env.INSTAGRAM_BUSINESS_ACCOUNT_ID_VEMOS
    ...
    const resolveInstagramBusinessAccountId = async (): Promise<string> => {
      ...
      if (businessAccountId) {
        ...
        console.warn(
          `[Instagram API] Provided INSTAGRAM_BUSINESS_ACCOUNT_ID_VEMOS is ${note}. Falling back to resolving via FACEBOOK_PAGE_ID_VEMOS.`,
        )
      }
          ...
          const responseBody: any = {
            ...
            suggestions: missingPermissions.length
              ? [
                  ...
                  "Alternatively, set a valid INSTAGRAM_BUSINESS_ACCOUNT_ID_VEMOS to avoid the Page lookup.",
                ]
              : undefined,
            timestamp: new Date().toISOString(),
          }
            ...
            for (const m of needsFallback) {
              try {
                ...
                if (Array.isArray(per?.data)) {
                  ...
                  // doesn't break the standard insights above.
                  if (m.media_type === "VIDEO") {
                    try {
