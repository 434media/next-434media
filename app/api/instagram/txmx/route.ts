  ...
  case "insights":
      ...
      case "stories":
        ...
        try {
          ...
          // Per-story insights in parallel. v22+ uses 'views' (replaces 'impressions');
          // 'navigation' returns a breakdown for swipe_forward/tap_back/tap_exit.
          const storiesWithInsights = await Promise.all(
            stories.map(async (story: any) => {
              try {
                ...
                // Navigation is an object breakdown (not a number)
                ...
                // Completion rate proxy: (views − exits) / views. IG doesn't expose
                // a direct completion metric per story.
