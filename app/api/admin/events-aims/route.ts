...
export async function GET() {
  try {
    ...
    console.error("Error fetching AIMS events:", error)
    return NextResponse.json(
      { error: "Failed to fetch AIMS events" },
      { status: 500 }
    )
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("Error creating AIMS event:", error)
    return NextResponse.json(
      { error: "Failed to create AIMS event" },
      { status: 500 }
    )
  }
}
