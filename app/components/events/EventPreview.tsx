...
export function EventPreview({ event, onViewDetails, className }: EventPreviewProps) {
  ...
  if (!isValidDate) {
    return (
      <div
        className="bg-white rounded-xl shadow-md border border-neutral-200 p-6 transition-all duration-300 hover:shadow-xl hover:scale-105 cursor-pointer group h-[350px]"
      ...
      >
        ...
        <h3 className="font-bold text-lg text-neutral-900 mb-3 group-hover:text-neutral-600 transition-colors line-clamp-2">
          {event.title}
        </h3>

        {event.description && <p className="text-sm text-neutral-600 line-clamp-2 mb-4">{event.description}</p>}

        <div className="flex justify-between items-center pt-3 border-t border-neutral-100">
          <span className="text-xs text-neutral-500">
            {event.organizer ? `By ${event.organizer}` : "Details available"}
          </span>
          <span className="text-xs font-medium text-neutral-600">View details</span>
        </div>
      </div>
    )
  }
    ...
    switch (urgency) {
      ...
      case "tomorrow":
        return {
          ...
          borderColor: "border-neutral-200 ring-1 ring-neutral-100",
          icon: Clock,
  ...
  const getCategoryColor = (category?: string) => {
    switch (category) {
      case "conference":
        return "from-neutral-500 to-red-500"
      case "workshop":
        return "from-neutral-500 to-neutral-500"
      case "meetup":
        return "from-yellow-500 to-neutral-500"
      case "networking":
  ...
  const getSourceIndicator = (source?: string) => {
    switch (source) {
      ...
      case "eventbrite":
        return { color: "bg-neutral-500", name: "Eventbrite" }
      case "luma":
        return { color: "bg-purple-500", name: "Luma" }
      default:
        return { color: "bg-neutral-500", name: "Manual" }
    }
  }
  ...
  return (
    <div
      className={cn(
        "mt-6 bg-white rounded-2xl shadow-md border border-neutral-200 p-6 cursor-pointer transition-all duration-300 hover:shadow-xl hover:scale-105 group relative h-[350px]",
    ...
    >
      ...
      <div className="p-4 space-y-3">
        ...
        {/* Description Preview */}
        {event.description && <p className="text-sm text-neutral-600 line-clamp-2 leading-relaxed">{event.description}</p>}

        {/* Footer */}
        <div className="flex md:flex-col md:items-start md:gap-1 items-center justify-between pt-2 border-t border-neutral-100">
          ...
          <div className="flex items-center gap-2">
            {event.url && (
              <ExternalLink className="h-4 w-4 text-neutral-400 group-hover:text-blue-500 transition-colors" />
            )}
            <span className="text-xs text-neutral-400 group-hover:text-neutral-600 transition-colors">View Details</span>
          </div>
        </div>
      </div>
    </div>
