...
export function MailchimpTopCampaignsTable({ data }: MailchimpTopCampaignsTableProps) {
  ...
  const getPerformanceIndicator = (openRate: number) => {
    if (openRate >= 25) return { icon: TrendingUp, color: "text-green-600", bg: "bg-green-50", label: "Excellent" }
    if (openRate >= 20) return { icon: TrendingUp, color: "text-blue-600", bg: "bg-blue-50", label: "Good" }
    if (openRate >= 15) return { icon: Minus, color: "text-amber-600", bg: "bg-amber-50", label: "Average" }
    return { icon: TrendingDown, color: "text-red-600", bg: "bg-red-50", label: "Poor" }
  }
  ...
  return (
    <div className="rounded-xl border border-neutral-200 bg-white overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-neutral-50/50">
              <th className="text-left py-2 px-3 font-medium text-neutral-600 text-xs uppercase tracking-wide">Campaign</th>
              <th className="text-left py-2 px-3 font-medium text-neutral-600 text-xs uppercase tracking-wide whitespace-nowrap">Sent</th>
              <th className="text-right py-2 px-3 font-medium text-neutral-600 text-xs uppercase tracking-wide">Sent</th>
              <th className="text-right py-2 px-3 font-medium text-neutral-600 text-xs uppercase tracking-wide">Opens</th>
              <th className="text-right py-2 px-3 font-medium text-neutral-600 text-xs uppercase tracking-wide">Clicks</th>
              <th className="text-right py-2 px-3 font-medium text-neutral-600 text-xs uppercase tracking-wide whitespace-nowrap">Open %</th>
              <th className="text-right py-2 px-3 font-medium text-neutral-600 text-xs uppercase tracking-wide whitespace-nowrap">Click %</th>
              <th className="text-center py-2 px-3 font-medium text-neutral-600 text-xs uppercase tracking-wide"></th>
            </tr>
          </thead>
            ...
            {campaigns.map((campaign, index) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Open Rate */}
                  <td className="py-2.5 px-3 text-right">
                    <span className={`font-medium tabular-nums text-xs ${openRate >= 20 ? 'text-green-600' : openRate >= 15 ? 'text-amber-600' : 'text-neutral-900'}`}>
                      {openRate.toFixed(1)}%
                    </span>
                  </td>
                  ...
                  {/* Click Rate */}
                  <td className="py-2.5 px-3 text-right">
                    <span className={`font-medium tabular-nums text-xs ${clickRate >= 3 ? 'text-green-600' : clickRate >= 2 ? 'text-amber-600' : 'text-neutral-900'}`}>
                      {clickRate.toFixed(1)}%
                    </span>
                  </td>
                  ...
                  <td className="py-2.5 px-3 text-center">
                    <button
                      ...
                      className="p-1.5 rounded-md text-neutral-400 hover:text-neutral-700 hover:bg-neutral-100 transition-colors opacity-0 group-hover:opacity-100"
      ...
      {campaigns.length > 0 && data.totalCampaigns > 15 && (
        <div className="border-t border-neutral-100 px-4 py-2 bg-neutral-50/30">
