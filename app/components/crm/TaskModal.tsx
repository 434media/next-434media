          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-6">
              ...
              <div className="pt-4 border-t border-gray-200 space-y-3">
                ...
                {/* Social Calendar Toggle */}
                <div className="flex items-center justify-between p-3 rounded-lg bg-gray-50 border border-gray-200">
                  <div className="flex items-center gap-3">
                    <div className={`p-2 rounded-lg ${formData.is_social_post ? 'bg-pink-100' : 'bg-gray-200'}`}>
                      <Calendar className={`w-4 h-4 ${formData.is_social_post ? 'text-pink-600' : 'text-gray-500'}`} />
                    </div>
                    <div>
                      <p className="text-sm font-medium text-gray-900">Link to Social Calendar</p>
                      <p className="text-xs text-gray-500">Schedule this task as a social media post</p>
                    </div>
                  </div>
                  <button
                    type="button"
                    onClick={() => onFormChange({ 
                      ...formData, 
                      is_social_post: !formData.is_social_post,
                      social_post_date: !formData.is_social_post ? formData.social_post_date : "",
                      social_platforms: !formData.is_social_post ? formData.social_platforms : []
                    })}
                    className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                      formData.is_social_post ? 'bg-pink-600' : 'bg-gray-300'
                    }`}
                  >
                    <span
                      className={`pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out ${
                        formData.is_social_post ? 'translate-x-5' : 'translate-x-0'
                      }`}
                    />
                  </button>
                </div>
                
                {/* Social Calendar Options - shown when toggle is enabled */}
                {formData.is_social_post && (
                  <div className="pl-12 space-y-3">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">Post Date</label>
                      <input
                        type="date"
                        value={formData.social_post_date}
                        onChange={(e) => onFormChange({ ...formData, social_post_date: e.target.value })}
                          ...
                          return (
                            <button
                              ...
                              onClick={() => {
                                ...
                                onFormChange({ ...formData, social_platforms: newPlatforms })
                              }}
