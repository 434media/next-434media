...
async function ensurePool(connString) {
    if (!pool)
    ...
    if (msg.jobType === 'scrape' && msg.urls?.length) {
        ...
        try {
            ...
            for (const d of core.candidates) {
                const upsertRes = await pool.query(`INSERT INTO leads (company_name, website_url, industry, location, contact_name, contact_title, email, phone, status, notes, source_url)
           ...
           ON CONFLICT (company_name) DO UPDATE SET
             ...
             updated_at = NOW();`, [d.company_name, d.website_url || null, d.industry || null, d.location || null, d.contact_name || null, d.contact_title || null, d.email || null, d.phone || null, d.source_url || null]);
                if (d.contacts?.length) {
                    ...
                    if (leadId) {
                        for (const c of d.contacts.slice(0, 25)) {
                            await pool.query(`INSERT INTO lead_contacts (lead_id, name, title, email, phone, linkedin_url, twitter_url)
                 VALUES ($1,$2,$3,$4,$5,$6,$7)`, [leadId, c.name, c.title || null, c.email || null, c.phone || null, c.linkedin_url || null, c.twitter_url || null]);
                        }
                    }
                }
            }
